...
def test_energy_uncertainty_uses_the_interval_the_trip_was_driven_under():
    ...
    anything it reads that is global — a constant, today's configuration —
    is applied retroactively to every trip ever driven. When EnergyRemaining
    went from sixty seconds to ten, that would have shrunk the whole
    history's error bar on a day nothing was measured, and the accuracy
    report sums these in quadrature.

    So the interval comes off the trip, measured from the gaps between its
    own readings when it closed. A trip without one is a trip closed before
    that was measured, and every one of those was driven at sixty seconds.
    """
    ...
    assert _energy_unc_kwh({**trip, "energy_sample_sec": 60.0}) == pytest.approx(0.136, abs=0.001)
    assert _energy_unc_kwh({**trip, "energy_sample_sec": 10.0}) == pytest.approx(0.023, abs=0.001)

    # No measurement, or an unusable one, falls back to the wide figure
    # rather than quietly claiming the narrow one.
    assert _energy_unc_kwh(trip) == pytest.approx(0.136, abs=0.001)
    assert _energy_unc_kwh({**trip, "energy_sample_sec": None}) == pytest.approx(0.136, abs=0.001)
    assert _energy_unc_kwh({**trip, "energy_sample_sec": 0}) == pytest.approx(0.136, abs=0.001)
    assert _energy_unc_kwh({**trip, "energy_sample_sec": "fast"}) == pytest.approx(0.136, abs=0.001)
    ...
    assert _energy_unc_kwh({"energy_kwh": 0.5, "duration_min": 60.0,
                            "energy_sample_sec": 10.0}) == pytest.approx(0.02, abs=0.0001)
