...
class _DeadZoneAtHomeClient(_SleepsAfterDrivingClient):
    """The real pattern: no signal at Home. The car goes unreachable a few
    hundred metres out, parks, sleeps, and is not heard from again until it
    has already set off — so no poll ever catches it stopped."""

    LAST_SEEN = (5.34620, 100.31130)   # ~0.36 km short of home
    NEXT_SEEN = (5.33650, 100.30950)   # ~0.73 km past it, already driving
    ...
    def vehicle_data(self, vid):
        if type(self).step < 3:
            d = super().vehicle_data(vid)
            d["drive_state"]["latitude"], d["drive_state"]["longitude"] = self.LAST_SEEN
            return d
        # Back online only once it is moving again, 1.10 km further on.
        return {
            "vin": self.VIN,
            "display_name": "Highland",
            "drive_state": {"timestamp": 1_760_500_000_000 + 300_000 + 3_600_000,
                            "shift_state": "D", "speed": 35,
                            "latitude": self.NEXT_SEEN[0], "longitude": self.NEXT_SEEN[1]},
            "charge_state": {"battery_level": 74, "battery_range": 370.0 / 1.60934,
                             "charging_state": "Disconnected"},
            "climate_state": {"outside_temp": 28},
            "vehicle_state": {"odometer": ODO_KM_TO_MI(10013.1),
                              "is_user_present": True, "locked": False},
            "vehicle_config": {"car_type": "model3"},
        }


def test_a_dead_zone_park_gives_each_trip_its_own_share(monkeypatch):
    """Regression for four consecutive arrivals home. With no signal at the
    destination the arrival correction can never run — it needs a poll while
    the car is stopped, and there isn't one — so the previous trip's last few
    hundred metres and the next trip's first few arrived as one lump on the
    departing trip. A named place between the two readings divides it."""
    ...
    monkeypatch.setattr(routes_mod, "_place_and_area",
                        lambda coords, session=None: ("Home", "Home"))
    try:
        with SessionLocal() as s:
            s.add(Place(name="DeadZoneHome", lat=5.34300, lon=100.31070,
                        radius_km=0.15, created_at=datetime(2026, 1, 1)))
            s.commit()
        closed_id, dist_before, _e, drives, _logged = _run_asleep_close(
            monkeypatch, _DeadZoneAtHomeClient)
        closed = next(d for d in drives if d.id == closed_id)
        # The sleep close left it at 12.0 km; the arrival owed to it is the
        # short leg of the 1.10 km stretch, not the whole thing.
        assert dist_before == 12.0
        assert 12.3 <= closed.distance_km <= 12.5
        assert closed.end_location == "Home"      # where it actually stopped
        assert closed.end_lost_km == 0.0          # measured now, not provisional
        # And the rest went to the departing trip rather than being counted
        # twice — total across both still matches the odometer.
        assert closed.distance_km < dist_before + 1.10
    finally:
        with SessionLocal() as s:
            for p in s.query(Place).filter(Place.name == "DeadZoneHome").all():
                s.delete(p)
            s.commit()
