...
def _clear_full_history_cache():
    """Module-level caches in app/api/routes.py survive between requests on
    purpose (that's the point of them), which means they also survive between
    tests unless cleared. _full_history() caches whole-history rows keyed by
    vehicle_id; _parked_readings() caches its SentryIndex the same way;
    _HEALTH_CHECKS_CACHE caches /api/health's promotion and continuity checks,
    unkeyed. Every test's ``session`` fixture is a fresh in-memory database
    that still numbers its first vehicle 1, so a cache entry left standing by
    one test is a stale answer handed to the next one — silently, since
    ...
    real (non-test) run started later in the same process — unlikely, but the
    failure mode is a wrong number with no error — never inherits a test's
    fixture data.
    """
    from app.api import routes

    routes._FULL_HISTORY_CACHE.clear()
    routes._PARKED_READINGS_CACHE.clear()
